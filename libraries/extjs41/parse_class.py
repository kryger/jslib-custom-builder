...
class ExtClassParser:
    ...
    def __parse(self):
      with open(self.class_file_location, 'r') as f:
        ...
        self.requires = self.__process_requires(contents, self.class_file_location)
        self.mixins = self.__process_mixins(contents, self.class_file_location)
        self.uses = self.__process_uses(contents, self.class_file_location)
        self.extends = self.__process_extends(contents)
        self.alternate_class_names = self.__process_alternate_class_names(contents, self.class_file_location)
        self.xtypes = self.__process_xtypes(contents, self.class_file_location)
        
        print '\nParsed module= %s\nclassname=%s\nextends=%s\nmixins=%s\nuses=%s\nrequires=%s\nalt=%s\nalias=%s' % (self.class_file_location, self.class_name, self.extends, self.mixins, self.uses, self.requires, self.alternate_class_names, self.xtypes)
    ...
    def __process_alternate_class_names(self, contents, module_path):
      ...
      assert len(alt_blocks) == 1, 'At most one \'alternateClassName\' property expected in the module, found: %i; offending file: %s' % (len(alt_blocks), module_path)
    ...
    def __process_mixins(self, contents, module_path):
      ...
      assert len(mixins_blocks) == 1, 'At most one \'mixins\' property expected in the module, found: %i; offending file: %s' % (len(mixins_blocks), module_path)
      mixins_array_string = mixins_blocks[0]

      # entire 'mixins' block squeezed into a single line
      # observable:Ext.util.Observable,animate:Ext.util.Animate,elementCt:Ext.util.ElementContainer,renderable:Ext.util.Renderable,state:Ext.state.Stateful
      mixins_array_string_sub = re.sub("[\s']", '', mixins_array_string)
      mixins = []
    ...
    def __process_uses(self, contents, module_path):
      ...
      assert len(uses_blocks) == 1, 'At most one \'uses\' property expected in the module, found: %i; offending file: %s' % (len(uses_blocks), module_path)
    ...
    def __process_xtypes(self, contents, module_path):
      ...
      matches = re.search(self.EXT_XTYPES_TOKEN_SINGLE, contents, re.MULTILINE)
      
      if not matches:
        matches = re.search(self.EXT_XTYPES_TOKEN_MULTI, contents, re.MULTILINE)
        if not matches:
          # no alias names
          return []
      
      require_blocks = matches.groups()
      assert len(require_blocks) == 1, 'At most one \'alias\' property expected in the module, found: %i; offending file: %s' % (len(require_blocks), module_path)
    ...
    def __process_requires(self, contents, module_path):
      matches = re.search(self.EXT_REQUIRES_TOKEN_MULTI, contents, re.DOTALL)
      
      # class has no own dependencies
      if not matches:
        matches = re.search(self.EXT_REQUIRES_TOKEN_SINGLE, contents, re.DOTALL)
        if not matches:
          return []
      
      require_blocks = matches.groups()
      
      assert len(require_blocks) == 1, 'At most one \'requires\' property expected in the module, found: %i; offending file: %s' % (len(require_blocks), module_path)
    ...
    def __process_extends(self, contents):
      matches = re.search(self.EXT_EXTENDS_TOKEN, contents, re.MULTILINE)
      if not matches:
        return None
        
      extend_blocks = matches.groups()
      
      assert len(extend_blocks) == 1, 'At most one \'extend\' property expected in the module, found: %i; offending file: %s' % (len(extend_blocks), module_path)

      return extend_blocks[0]
    
    # extract name of the class in the module
    def __process_class_name(self, contents):
      matches = re.search(self.EXT_DEFINE_TOKEN, contents, re.MULTILINE)
      if not matches:
         matches = re.search(self.EXT_DEFINE_TOKEN_DOUBLEQUOTE, contents, re.MULTILINE)
         if not matches:
