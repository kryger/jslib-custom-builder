...
class ExtBuilder:
    ...
    def __get_embedded_classes(self):
        ...
        for line in open('%s/%s.js' % (self.ext_path,
                         self.in_build_file)).readlines():
    ...
    def __get_canonical_name(self, component_name):
        for component in self.ext_hierarchy:
            if component_name == component['class_name'] \
                or component_name in component['alt_class_names'] \
                or component_name in component['xtypes']:
...
''' \
    ...
    def __extract_used_modules(self):
        ...
        for (dirpath, dirnames, filenames) in \
            os.walk(self.js_project_path):
            for filename in filenames:
                jsfile = '%s/%s' % (dirpath, filename)

                with open(jsfile, 'r') as f:
                    contents = f.read()
                    class_matches = re.findall(EXT_CREATE_TOKEN,
                            contents, re.DOTALL)
                    if class_matches:
                    ...
                    # at this point match contains XXX in Ext.create(XXX)
                            ...
                            ext_component_quoted = match.split(','
                                    )[0].strip()
                    ...
                    # add, getting rid of quotes
                            ...
                            canonical = \
                                self.__get_canonical_name(unquoted)
                            if canonical not in ext_components:
                                print 'detected Ext.create element:', \
                                    canonical
                                ext_components.add(canonical)

                    for pattern in [EXT_PROXY_TOKEN, EXT_FIELDS_TOKEN]:
                        proxy_matches = re.findall(pattern, contents,
                                re.DOTALL)
                        if proxy_matches:
                            for match in proxy_matches:
                                for typ in re.finditer(EXT_TYPE_TOKEN,
                                        match):
                                    canonical = \
    self.__get_canonical_name(typ.group(1))
                                    if canonical not in ext_components:
                                        print 'detected proxy/reader/grid field type element:', \
    canonical
                                        ext_components.add(canonical)

                    patterns = [EXT_XTYPE_TOKEN,
                                EXT_DEFAULT_TYPE_TOKEN,
                                EXT_LAYOUT_TOKEN]
                    ...
                    for pattern in patterns:
                        ...
                        # group (1) is the capturing group
                            ...
                            canonical = \
                                self.__get_canonical_name(def_type.group(1))
                            if canonical not in ext_components:
