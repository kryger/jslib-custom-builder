...
if __name__ == '__main__':
    arg_parser = \
        argparse.ArgumentParser(description='jslib custom builder')
    arg_parser.add_argument('-sourcedir',
                            help='Path to the directory containing the JS files'
                            , action='store', dest='source_dir',
                            required=True)
    arg_parser.add_argument('-extdir',
                            help='Path to the ExtJS distribution folder'
                            , action='store', dest='ext_dir',
                            required=True)
    params = vars(arg_parser.parse_args())
    ext_builder = ExtBuilder(params['source_dir'], params['ext_dir'],
                             EXT_BUILD_FILE_IN)

    need_these = ['Ext.picker.Date', 'Ext.EventObject',
                  'Ext.EventObjectImpl', 'Ext.dom.CompositeElement',
                  'Ext.util.TaskRunner']  # 'Ext.dom.Element',
...
unused=''', unused_classes
    b = BuildTrimmer(unused_classes, params['ext_dir'] + '/%s.js'
                     % EXT_BUILD_FILE_IN)
    b.trim()
